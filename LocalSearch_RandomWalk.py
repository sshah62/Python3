...
# DSA 5113, Spring 2017, Homework 4, Problem 5 (Random Walk)
...
while done == 0:
    ...
    Neighborhood1 = neighborhood(x_curr)  # create a list of all neighbors in the neighborhood of x_curr
    val = myPRNG.random()
    if p > val:
        for s in Neighborhood1:  # evaluate every member in the neighborhood of x_curr
    ...
    # Random Walk
        ...
        s = Neighborhood1[myPRNG.randint(0, len(Neighborhood1)-1)]  # Forcing 'random' to select another solution in the neighborhood
        Neighborhood2 = neighborhood(s)
    ...
    if solutionsChecked > Max_it:  # if there were no improving solutions in the neighborhood
        done = 1
    else:
        x_curr = x_best[:]  # else: move to the neighbor solution and continue
